...
from dlt.common.destination import Destination, TDestinationReferenceArg, TLoaderFileFormat
...
from dlt.common.schema import Schema, TTableSchemaColumns
...
def expect_load_file(
    ...
    if chain_jobs := client.create_table_chain_completed_followup_jobs(
        [table],
        [LoadJobInfo("completed_jobs", full_path, 0, 0, 0, job.job_file_info(), "")],  # type: ignore[arg-type]
    ):
...
def normalize_storage_table_cols(
    ...
    storage_table = normalize_table_identifiers(
        new_table(table_name, columns=cols.values()),
        schema.naming,  # type: ignore[arg-type]
    )
